...
class PESQMetric(BaseMetric):
    def __init__(self, device="auto", *args, **kwargs):
        ...
        self.device = device
        self.pesq = PerceptualEvaluationSpeechQuality(fs=8000, mode="nb").to(self.device)

    def pesq_score(self, preds: torch.Tensor, refs: torch.Tensor):
        """
        ...
        for batch_idx in range(B):
            first_score = self.pesq(preds[:, batch_idx, :], refs[:, batch_idx, :])
            second_score = self.pesq(preds_flipped[:, batch_idx, :], refs[:, batch_idx, :])
    ...
    def __call__(self, output_audios: torch.Tensor, s1: torch.Tensor, s2: torch.Tensor, mix: torch.Tensor, **kwargs):
        ...
        return self.pesq_score(output_audios, refs)
