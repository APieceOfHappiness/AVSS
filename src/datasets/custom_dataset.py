...
class CustomDirDataset(BaseDataset):
    def __init__(
        self, dataset_path, dataset_length, audio_ref, video_ref, refs_cnt, override=False, *args, **kwargs
    ):
        index_path = ROOT_PATH / dataset_path / "index" / "index.json"

        if index_path.exists() and not override:
            index = read_json(str(index_path))
        else:
            index = self._create_index(dataset_path, dataset_length, audio_ref, video_ref, refs_cnt)
