...
class DlaDataset(BaseDataset):
    def __init__(
        self, dataset_path, dataset_length, audio_ref, video_ref, name="train", override=False, *args, **kwargs
    ):
        ...
        if index_path.exists() and not override:
            ...
            index = self._create_index(dataset_path, dataset_length, audio_ref, video_ref, name)

        super().__init__(index, *args, **kwargs)

    def _create_index(self, dataset_path, dataset_length, audio_ref, video_ref, name):
        index_path = ROOT_PATH / dataset_path / "index" / name
        audio_path = ROOT_PATH / dataset_path / "audio" / name
        video_path = ROOT_PATH / dataset_path / "mouths"
